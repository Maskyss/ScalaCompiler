...
class SymanticWriter {




  val writer = new PrintWriter(fileName+".lst", "UTF-8")
    ...
    if(listBuf.length==2)
      for(go<-listBuf){
        if(go.typeOfLexem == "User ident"){
          ...
          printList(listBuf)
          writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
          writer.println("Expected: instruction or directive")
        }
      }
    else {
  ...
  def printList(listBuf: List[syntaxToken]) = {
    writer.print("\t\t")
    for(go<-listBuf)writer.print(go.token+" ")
    writer.println()
  }
  ...
  def printHex(i:Int): Unit ={
    ...
    if(i.toHexString.length==1){
      ...
      writer.print(bytesHex.toString().toUpperCase)
    }
    ...
    if(i.toHexString.length>4) writer.println("Memory")
  }
  ...
  def DataVariables(listBuf: List[syntaxToken]):Int = {
    ...
    if(listBuf.length==1){
      if(listBuf(0).typeOfLexem=="User ident"){
        ...
        printList(listBuf)
        writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
        writer.println("Operand expected")
      }
      else {
        printList(listBuf)
        writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
        writer.println("Extra characters on line")
      }
    }
    ...
    listBuf(0).token.toLowerCase match{
      case "finit" =>{
        if(listBuf.length==1){
          ...
          printList(listBuf)
          writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
          writer.println("Reserved word used as symbol: FINIT")
        }
        else {
          printHex(bytesInt)
          printList(listBuf)
          ...
          if(faddF(listBuf)){
            if(listBuf(3).typeOfLexem == "User ident"){
              if(nameIdentData.filter(_.name==listBuf(3).token).isEmpty){
                writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
                writer.println("Symbol not defined:"+listBuf(3).token)
              }
            }
            if(listBuf(5).typeOfLexem == "User ident"){
              if(nameIdentData.filter(_.name==listBuf(5).token).isEmpty){
                writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
                writer.println("Symbol not defined:"+listBuf(5).token)
              }
            }
          }
          else{
    ...
    for (listBuf <- SyntaxList) {
      ...
      if(listBuf(0).token.toLowerCase=="end"&&(listBuf==SyntaxList.last)){
        ...
        if((DataBegin.isEmpty||DataBegin!=DataEnd)){
          printList(listBuf)
          writer.print(fileName +".asm ("+listBuf(0).row+")"+ "\t")
          writer.println("Block nesting error")}
        else {
          printHex(bytesInt)
          printList(listBuf)
        }
      ...
      if (!DataExist&&(listBuf.filter(_.token.toLowerCase.contains("ends")).isEmpty )&&(!DataExist&&(listBuf.filter(_.token.toLowerCase.contains("end")).isEmpty )))
        writer.println("Data emitted with no segment")
    }
    if(SyntaxList.last.filter(_.token.toLowerCase=="end").isEmpty) {
      writer.print(fileName +".asm ("+(LexicalList.length+1)+")"+ "\t")
      writer.println("End of file, no END directive")
    }

    if(DataEnd!=DataBegin&&(DataBegin.length>DataEnd.length)){
      writer.println("Open segments")
    }
  }
